...
class Colaborador
{
    ...
    public static function all(array $filters = []): array
    {
        ...
        $sql = 'SELECT c.id, c.nome, c.ativo, c.created_at,
                       c.matricula, c.salario_atual, c.data_admissao, c.data_inicio_cargo,
    ...
    public static function updateRhData(int $id, array $data): array
    {
        ...
        $stmtUnique = Database::conn()->prepare('SELECT id FROM colaboradores WHERE matricula = ? AND id <> ? LIMIT 1');
        $stmtUnique->execute([$matricula, $id]);
        if ($stmtUnique->fetch(PDO::FETCH_ASSOC)) {
            return ['ok' => false, 'error' => 'Já existe outro colaborador cadastrado com essa matrícula.'];
        }
        ...
        $stmt->execute([$matricula, $salarioAtual, $dataAdmissao, $dataInicioCargo, $id]);
