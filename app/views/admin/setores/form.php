...
<div class="mx-auto max-w-6xl space-y-6">
  <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
    <div>
      ...
      <p class="text-fluid-subtitle text-gray-600">Gerencie o cadastro do setor e os cargos habilitados para este relacionamento.</p>
    </div>
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
      <form method="post" action="<?= $base . $formAction ?>" class="space-y-5">
        ...
        <div>
          ...
          <select name="empresa_id" required class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
          ...
          <p class="mt-2 text-xs text-slate-500">Somente empresas ativas podem ser selecionadas para novos vínculos.</p>
        </div>
      ...
      <div class="space-y-6">
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <div class="flex items-center justify-between gap-3">
            <div>
              <h2 class="text-lg font-semibold text-slate-900">Cargos vinculados</h2>
              <p class="text-sm text-slate-500">Remova vínculos individualmente sem alterar o cadastro principal.</p>
            </div>
            <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600"><?= count($linkedCargos) ?> vínculo(s)</span>
          </div>
          ...
          <div class="mt-4 space-y-3">
            <?php foreach ($linkedCargos as $cargo): ?>
              <div class="flex flex-col gap-3 rounded-xl border border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div class="font-semibold text-slate-900"><?= Security::e($cargo['nome']) ?></div>
                  <div class="text-xs text-slate-500">Slug: <?= Security::e($cargo['slug']) ?></div>
                </div>
        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <h2 class="text-lg font-semibold text-slate-900">Associar cargos</h2>
          <p class="mt-1 text-sm text-slate-500">Selecione um ou mais cargos disponíveis para este setor.</p>

          <form method="post" action="<?= $base ?>/admin/setores/<?= (int)$item['id'] ?>/cargos/vincular" class="mt-4 space-y-4">
            <input type="hidden" name="csrf" value="<?= Security::e($csrf ?? Security::csrfToken()) ?>">
            <div>
              <label class="mb-2 block text-sm font-medium text-slate-700">Cargos disponíveis</label>
              <select name="cargo_ids[]" multiple size="10" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
                <?php foreach ($availableCargos as $cargo): ?>
                  <option value="<?= (int)$cargo['id'] ?>"><?= Security::e($cargo['nome']) ?></option>
                <?php endforeach; ?>
              </select>
              <p class="mt-2 text-xs text-slate-500">Use Ctrl/Cmd para selecionar múltiplos cargos.</p>
            </div>
            <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-blue-900 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-blue-950">
              Vincular cargos
            </button>
